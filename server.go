...
type Config struct {
	...
	DeletionPeriod int64  `json:"deletion_period"`
	ScrubOnReboot  bool   `json:"scrub_on_reboot"`
...
func deletePeriod(fname string) {
	if config.DeletionPeriod != 0 {
		fmt.Println("waiting to delete " + fname)
		time.Sleep(time.Duration(config.DeletionPeriod) * time.Second)
		os.Remove(fname)
...
func main() {
	configName := flag.String("config", "server.conf.json", "Configuration file")
	flag.Parse()

	config = readConfig(*configName)
	validateConfig(config)

	if config.ScrubOnReboot {
		os.RemoveAll(config.Path.I)
		os.MkdirAll(config.Path.I, 0755)
		os.Create(config.Path.I + "/.placeholder")
	}
